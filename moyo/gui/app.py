...
class WorkerThread(QThread):
    """Worker thread for running long operations off the GUI thread."""
...
class GatherPublicSourcesTab(QWidget):
    ...
    def init_ui(self):
        ...
        desc = QLabel(
            "Gather public sources for building public FAISS indices. "
            "Use moyo-gather on the command line for headless operation."
        )
        desc.setWordWrap(True)
        layout.addWidget(desc)

        placeholder = QLabel("Public source gathering UI coming soon.")
...
class BuildPublicCorpusTab(QWidget):
    ...
    def init_ui(self):
        ...
        desc = QLabel("Gather public sources and build public FAISS indices.")
        desc.setWordWrap(True)
        layout.addWidget(desc)

        placeholder = QLabel("Public index building UI coming soon.")
        placeholder.setAlignment(Qt.AlignCenter)
        placeholder.setStyleSheet("color: gray; font-style: italic;")
        layout.addWidget(placeholder)
...
class BarrierProbeTab(QWidget):
    ...
    def init_ui(self):
        ...
        desc = QLabel(
            "Run barrier probing analysis. "
            "Use moyo-probe on the command line for full control."
        )
        desc.setWordWrap(True)
        layout.addWidget(desc)

        placeholder = QLabel("Barrier probing UI coming soon.")
        placeholder.setAlignment(Qt.AlignCenter)
        placeholder.setStyleSheet("color: gray; font-style: italic;")
        layout.addWidget(placeholder)
...
class VisualizationTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.show_distance_lines_checkbox = QCheckBox(
            "Show distance lines between closest points"
        )
        ...
        export_path_layout.addWidget(self.viz_export_path_label)
        export_path_layout.addWidget(select_viz_export_btn)
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            self.viz_expand_btn.clicked.connect(
                lambda: self.toggle_plot_size(self.canvas, viz_canvas_group)
            )
            ...
            self.heatmap_expand_btn.clicked.connect(
                lambda: self.toggle_plot_size(self.heatmap_canvas, heatmap_canvas_group)
            )
    ...
    def generate_visualization(self):
        ...
        try:
            ...
            private_vectors = self.private_index.index.reconstruct_n(
            ...
            public_vectors = self.public_index.index.reconstruct_n(
                0, self.public_index.get_vector_count()
            )

            self.output_text.append(
                f"Loaded {len(private_vectors)} private and {len(public_vectors)} public vectors"
            )
            ...
            self.output_text.append("Applying MDS dimensionality reduction...")
            ...
            if (
                self.show_distance_lines_checkbox.isChecked()
                and len(private_coords) > 0
                and len(public_coords) > 0
            ):
                try:
                    threshold = float(self.distance_threshold_input.text())
                    for i, private_point in enumerate(private_coords):
                        closest_idx = np.argmin(cross_distances[i])
                        dist = cross_distances[i][closest_idx]
                        if dist <= threshold:
                            pub_pt = public_coords[closest_idx]
                            lw = max(0.5, 3.0 - dist * 5.0)
                            alpha = max(0.2, 1.0 - dist * 2.0)
                            ax.plot(
                                [private_point[0], pub_pt[0]],
                                [private_point[1], pub_pt[1]],
                                "g-", alpha=alpha, linewidth=lw,
                            )
                    self.output_text.append(
                        f"Drew distance lines for points within cosine distance {threshold}"
                    )
                except Exception as e:
                    self.output_text.append(f"Error drawing distance lines: {e}")

            if (
                self.show_clustering_checkbox.isChecked()
                and len(private_coords) > 0
                and len(public_coords) > 0
            ):
                try:
                    ...
                    private_avg = np.mean(cross_distances, axis=1)
                    public_avg = np.mean(cross_distances, axis=0)
                    all_distances = np.concatenate([private_avg, public_avg]).reshape(-1, 1)

                    n_clusters = int(self.num_clusters_input.text())
                    labels = KMeans(n_clusters=n_clusters, random_state=42).fit_predict(all_distances)
                    private_labels = labels[: len(private_coords)]
                    public_labels = labels[len(private_coords):]
                    colors = plt.cm.Set3(np.linspace(0, 1, n_clusters))

                    ax.clear()
                    for i in range(n_clusters):
                        mask = private_labels == i
                        if np.any(mask):
                            ax.scatter(
                                private_coords[mask, 0], private_coords[mask, 1],
                                c=[colors[i]], label=f"Private Cluster {i + 1}",
                                alpha=0.7, s=50, marker="o",
                            )
                        mask = public_labels == i
                        if np.any(mask):
                            ax.scatter(
                                public_coords[mask, 0], public_coords[mask, 1],
                    ...
                    for i in range(n_clusters):
                        ...
                        avg_d = float(np.mean(all_distances[labels == i]))
                        self.output_text.append(
                            f"Cluster {i + 1}: {count} points, avg distance: {avg_d:.3f}"
                        )
                    self.output_text.append(
                        f"Applied distance-based clustering with {n_clusters} clusters"
                    )
                except ImportError:
                    self.output_text.append("Warning: sklearn not available for clustering")
                except Exception as e:
                    self.output_text.append(f"Error applying clustering: {e}")
            else:
            ...
            self.canvas.draw()
            self.output_text.append(f"Visualization generated ({algorithm.upper()})")
            self.output_text.append(f"Private: {len(private_coords)} (blue), Public: {len(public_coords)} (red)")
            self.viz_export_btn.setEnabled(True)

        except Exception as e:
            import traceback
            self.output_text.append(f"Error generating visualization: {e}")
            self.output_text.append(traceback.format_exc())
            QMessageBox.critical(self, "Error", f"Failed to generate visualization: {e}")
    ...
    def export_visualization(self):
        ...
        try:
            ...
            self.output_text.append(f"Error exporting: {e}")
    ...
    def generate_distance_heatmap(self):
        ...
        try:
            ...
            if self.private_index is None or self.public_index is None:
                QMessageBox.warning(self, "Warning", "Load both indices first")
                return
            ...
            x_coords, y_coords, dist_vals = [], [], []
            for i, pp in enumerate(private_coords):
                for j, qp in enumerate(public_coords):
                    x_coords.append((pp[0] + qp[0]) / 2)
                    y_coords.append((pp[1] + qp[1]) / 2)
                    dist_vals.append(cross_distances[i, j])

            x_coords = np.array(x_coords)
            y_coords = np.array(y_coords)
            dist_vals = np.array(dist_vals)

            x_pad = (x_coords.ptp()) * 0.1
            y_pad = (y_coords.ptp()) * 0.1
            x_bins = np.linspace(x_coords.min() - x_pad, x_coords.max() + x_pad, 50)
            y_bins = np.linspace(y_coords.min() - y_pad, y_coords.max() + y_pad, 50)

            heatmap, x_edges, y_edges = np.histogram2d(
                x_coords, y_coords, bins=[x_bins, y_bins], weights=dist_vals
            )
            count_hist, _, _ = np.histogram2d(x_coords, y_coords, bins=[x_bins, y_bins])
            heatmap = np.divide(
                heatmap, count_hist, out=np.zeros_like(heatmap), where=count_hist > 0
            )
            ...
            ax.set_ylabel("Dimension 2")
            ax.set_title(
                f"Distance Density Heatmap ({algorithm.upper()}) — "
                "red areas show high private-public proximity"
            )
            ax.grid(True, alpha=0.3)
            ax.set_xticklabels([])
            ax.set_yticklabels([])
            ...
            self.output_text.append(
                f"Heatmap generated — distance range: "
                f"{cross_distances.min():.3f}–{cross_distances.max():.3f}, "
                f"mean: {cross_distances.mean():.3f}"
            )
            self.heatmap_export_btn.setEnabled(True)

        except Exception as e:
            import traceback
            self.output_text.append(f"Error generating heatmap: {e}")
            self.output_text.append(traceback.format_exc())
    ...
    def export_heatmap(self):
        ...
        try:
            heatmap_path = (
                ...
                .replace(".pdf", "_heatmap.pdf")
                .replace(".svg", "_heatmap.svg")
            )
            self.heatmap_figure.savefig(heatmap_path, dpi=300, bbox_inches="tight")
            self.output_text.append(f"Heatmap exported to {heatmap_path}")
            QMessageBox.information(self, "Success", f"Heatmap exported to {heatmap_path}")
        except Exception as e:
            self.output_text.append(f"Error exporting heatmap: {e}")
            QMessageBox.critical(self, "Error", f"Failed to export heatmap: {e}")

    def toggle_plot_size(self, canvas, group_box):
        if canvas.size().width() <= 600:
