...
**Fuzz modes:** `basic` (original / paraphrase, English only) or
`multilingual` (original / paraphrase per language; defaults
Spanish, French, Mandarin Chinese; answers translated back). ``abstract``,
``translate``, ``typo`` and ``shuffle`` are optional a la
carte. See
